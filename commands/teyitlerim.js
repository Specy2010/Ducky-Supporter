...
module.exports = {
    ...
    run: async(client, message, args) => {
        if (!client.config.mods.some(id => message.member.roles.cache.has(id))) {
            return message.channel.send(new MessageEmbed().setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true })).setDescription("Komutu kullanan kullanıcıda yetki bulunmamakta!").setFooter('Lucas Was Here!').setColor('2F3136')).then(x => x.delete({timeout: 3000})).then(message.react(client.config.no));
        }
